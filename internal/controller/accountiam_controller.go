...
// CacheManager manages frequently accessed data to reduce API calls
...
// GetCachedSecretData retrieves secret data from cache or API
func (cm *CacheManager) GetCachedSecretData(ctx context.Context, client client.Client, secretName, namespace, key string) (string, error) {
	cacheKey := fmt.Sprintf("%s/%s", namespace, secretName)

	cm.cacheMux.RLock()
	if secretData, exists := cm.secretCache[cacheKey]; exists {
		if lastUpdate, hasUpdate := cm.lastUpdate[cacheKey]; hasUpdate && time.Since(lastUpdate) < cm.ttl {
			if data, keyExists := secretData[key]; keyExists {
				cm.cacheMux.RUnlock()
				return data, nil
			}
		}
	...
	data, err := utils.GetSecretData(ctx, client, secretName, namespace, key)
	if err != nil {
		return "", err
	}

	cm.cacheMux.Lock()
	if cm.secretCache[cacheKey] == nil {
		cm.secretCache[cacheKey] = make(map[string]string)
	}
	cm.secretCache[cacheKey][key] = data
	cm.lastUpdate[cacheKey] = time.Now()
	cm.cacheMux.Unlock()
