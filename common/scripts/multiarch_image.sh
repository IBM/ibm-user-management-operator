...
IMAGE_TAG_BASE=${1}
VERSION=${2-"$(git describe --exact-match 2> /dev/null || git describe --match=$(git rev-parse --short=8 HEAD) --always --dirty --abbrev=8)"}
RELEASE_VERSION=${3}
...
do
    ...
    do
        echo "Checking image '${IMAGE_TAG_BASE}'-'${arch}':'${VERSION}'..."
        ${CONTAINER_CLI} manifest inspect "${IMAGE_TAG_BASE}"-"${arch}":"${VERSION}" && break
        sleep "${RETRY_INTERVAL}"
        if [ "${i}" -eq "${MAX_PULLING_RETRY}" ]; then
            echo "Failed to found image '${IMAGE_TAG_BASE}'-'${arch}':'${VERSION}'!!!"
            exit 1
        fi
    done
done
...
${CONTAINER_CLI} manifest create "${IMAGE_TAG_BASE}":"${RELEASE_VERSION}" \
    "${IMAGE_TAG_BASE}"-amd64:"${VERSION}" \
    "${IMAGE_TAG_BASE}"-ppc64le:"${VERSION}" \
    "${IMAGE_TAG_BASE}"-s390x:"${VERSION}"
